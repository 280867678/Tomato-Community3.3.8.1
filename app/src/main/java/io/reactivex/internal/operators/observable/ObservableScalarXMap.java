...
public final class ObservableScalarXMap {
    public static <T, R> boolean tryScalarXMapSubscribe(ObservableSource<T> observableSource, Observer<? super R> observer, Function<? super T, ? extends ObservableSource<? extends R>> function) {
        if (observableSource instanceof Callable) {
            try {
                ...
                try {
                    ObservableSource<? extends R> mo6755apply = function.mo6755apply(obj);
                    ObjectHelper.requireNonNull(mo6755apply, "The mapper returned a null ObservableSource");
                    ObservableSource<? extends R> observableSource2 = mo6755apply;
    ...
    public static final class ScalarXMapObservable<T, R> extends Observable<R> {
        ...
        public void subscribeActual(Observer<? super R> observer) {
            try {
                ObservableSource<? extends R> mo6755apply = this.mapper.mo6755apply((T) this.value);
                ObjectHelper.requireNonNull(mo6755apply, "The mapper returned a null ObservableSource");
                ObservableSource<? extends R> observableSource = mo6755apply;
                if (observableSource instanceof Callable) {
                    try {
    ...
    public static final class ScalarDisposable<T> extends AtomicInteger implements QueueDisposable<T>, Runnable {
        ...
        @Override // io.reactivex.internal.fuseable.SimpleQueue
        /* renamed from: poll */
        public T mo6754poll() throws Exception {
            if (get() == 1) {
                lazySet(3);
                return this.value;
            }
