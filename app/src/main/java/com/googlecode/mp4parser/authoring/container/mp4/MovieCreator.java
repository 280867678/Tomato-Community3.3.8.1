    ...
    public static Movie build(DataSource dataSource) throws IOException {
        ...
        for (TrackBox trackBox : isoFile.getMovieBox().getBoxes(TrackBox.class)) {
            ...
            if (schemeTypeBox != null && (schemeTypeBox.getSchemeType().equals("cenc") || schemeTypeBox.getSchemeType().equals("cbc1"))) {
                movie.addTrack(new CencMp4TrackImplImpl(dataSource.toString() + "[" + trackBox.getTrackHeaderBox().getTrackId() + "]", trackBox, new IsoFile[0]));
            } else {
                movie.addTrack(new Mp4TrackImpl(dataSource.toString() + "[" + trackBox.getTrackHeaderBox().getTrackId() + "]", trackBox, new IsoFile[0]));
            }
        }
