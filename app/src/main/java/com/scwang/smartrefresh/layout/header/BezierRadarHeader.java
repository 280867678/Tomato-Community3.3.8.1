...
public class BezierRadarHeader extends InternalAbstract implements RefreshHeader {
    ...
    protected void drawDot(Canvas canvas, int i, int i2) {
        ...
        if (this.mDotAlpha > 0.0f) {
            ...
            while (i3 < 7) {
                ...
                this.mPaint.setAlpha((int) (this.mDotAlpha * (f4 - ((Math.abs(f10) / 7.0f) * f8)) * 255.0f * (1.0d - (1.0d / Math.pow((px2dp / 800.0d) + 1.0d, 15.0d)))));
    ...
    public void onStateChanged(@NonNull RefreshLayout refreshLayout, @NonNull RefreshState refreshState, @NonNull RefreshState refreshState2) {
        int i = C30731.$SwitchMap$com$scwang$smartrefresh$layout$constant$RefreshState[refreshState2.ordinal()];
        if (i == 1 || i == 2) {
            this.mDotAlpha = 1.0f;
            this.mRadarScale = 0.0f;
