...
public final class TypeFactory implements Serializable {
                ...
                if (javaType.isContainerType()) {
                    if (javaType.isMapLikeType()) {
                        if (cls == HashMap.class || cls == LinkedHashMap.class || cls == EnumMap.class || cls == TreeMap.class) {
                            _fromClass = _fromClass(null, cls, TypeBindings.create(cls, javaType.mo6146getKeyType(), javaType.mo6160getContentType()));
                        }
                    } else if (javaType.isCollectionLikeType()) {
                        if (cls == ArrayList.class || cls == LinkedList.class || cls == HashSet.class || cls == TreeSet.class) {
                            _fromClass = _fromClass(null, cls, TypeBindings.create(cls, javaType.mo6160getContentType()));
    ...
    public CollectionType constructCollectionType(Class<? extends Collection> cls, JavaType javaType) {
        ...
        if (createIfNeeded.isEmpty() && javaType != null) {
            JavaType mo6160getContentType = collectionType.findSuperType(Collection.class).mo6160getContentType();
            if (!mo6160getContentType.equals(javaType)) {
                throw new IllegalArgumentException(String.format("Non-generic Collection class %s did not resolve to something with element type %s but %s ", ClassUtil.nameOf(cls), javaType, mo6160getContentType));
            }
        }
    ...
    public MapType constructMapType(Class<? extends Map> cls, JavaType javaType, JavaType javaType2) {
        ...
        if (createIfNeeded.isEmpty()) {
            ...
            JavaType mo6146getKeyType = findSuperType.mo6146getKeyType();
            if (!mo6146getKeyType.equals(javaType)) {
                throw new IllegalArgumentException(String.format("Non-generic Map class %s did not resolve to something with key type %s but %s ", ClassUtil.nameOf(cls), javaType, mo6146getKeyType));
            }
            JavaType mo6160getContentType = findSuperType.mo6160getContentType();
            if (!mo6160getContentType.equals(javaType2)) {
                throw new IllegalArgumentException(String.format("Non-generic Map class %s did not resolve to something with value type %s but %s ", ClassUtil.nameOf(cls), javaType2, mo6160getContentType));
            }
        }
        return mapType;
    }
