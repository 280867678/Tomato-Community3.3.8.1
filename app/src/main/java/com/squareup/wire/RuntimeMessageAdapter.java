...
final class RuntimeMessageAdapter<M extends Message<M, B>, B extends Message.Builder<M, B>> extends ProtoAdapter<M> {
    ...
    @Override // com.squareup.wire.ProtoAdapter
    /* renamed from: decode */
    public M mo6533decode(ProtoReader protoReader) throws IOException {
            ...
            if (nextTag != -1) {
                ...
                if (fieldBinding != null) {
                    try {
                        ...
                        fieldBinding.value(newBuilder, singleAdapter.mo6533decode(protoReader));
                    } catch (ProtoAdapter.EnumConstantNotFoundException e) {
                        newBuilder.addUnknownField(nextTag, FieldEncoding.VARINT, Long.valueOf(e.value));
                    }
                } else {
                    FieldEncoding peekFieldEncoding = protoReader.peekFieldEncoding();
                    newBuilder.addUnknownField(nextTag, peekFieldEncoding, peekFieldEncoding.rawProtoAdapter().mo6533decode(protoReader));
                }
            } else {
                protoReader.endMessage(beginMessage);
                return (M) newBuilder.mo6479build();
            }
        }
    }
}
