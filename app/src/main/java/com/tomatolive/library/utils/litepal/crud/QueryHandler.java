...
public class QueryHandler extends DataHandler {
    ...
    public <T> T onFindFirst(Class<T> cls, boolean z) {
        List<T> query = query(cls, null, null, null, null, null, DatabaseFieldConfigLoader.FIELD_NAME_ID, "1", getForeignKeyAssociations(cls.getName(), z));
        if (query.size() > 0) {
            return query.get(0);
        }
        return null;
    ...
    public <T> List<T> onFindAll(Class<T> cls, boolean z, long... jArr) {
        if (isAffectAllLines(jArr)) {
            return query(cls, null, null, null, null, null, DatabaseFieldConfigLoader.FIELD_NAME_ID, null, getForeignKeyAssociations(cls.getName(), z));
        }
        return query(cls, null, getWhereOfIdsWithOr(jArr), null, null, null, DatabaseFieldConfigLoader.FIELD_NAME_ID, null, getForeignKeyAssociations(cls.getName(), z));
    }
