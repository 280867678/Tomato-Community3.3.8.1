...
public class DrawableTransformation implements Transformation<Drawable> {
    ...
    public Resource<Drawable> transform(@NonNull Context context, @NonNull Resource<Drawable> resource, int i, int i2) {
        ...
        Resource<Bitmap> convert = DrawableToBitmapConverter.convert(bitmapPool, mo5902get, i, i2);
        if (convert == null) {
            if (!this.isRequired) {
                return resource;
            }
            throw new IllegalArgumentException("Unable to convert " + mo5902get + " to a Bitmap");
        }
