...
public class LifecycleRegistry extends Lifecycle {
    ...
    static Lifecycle.State getStateAfter(Lifecycle.Event event) {
        switch (C00051.$SwitchMap$android$arch$lifecycle$Lifecycle$Event[event.ordinal()]) {
            case 1:
            case 2:
    ...
    private static Lifecycle.Event downEvent(Lifecycle.State state) {
        int i = C00051.$SwitchMap$android$arch$lifecycle$Lifecycle$State[state.ordinal()];
        if (i != 1) {
            if (i == 2) {
