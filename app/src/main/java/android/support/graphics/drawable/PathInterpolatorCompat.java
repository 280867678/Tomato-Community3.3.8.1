...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        this.f22mX = new float[min];
        this.f23mY = new float[min];
        float[] fArr = new float[2];
        for (int i2 = 0; i2 < min; i2++) {
            pathMeasure.getPosTan((i2 * length) / (min - 1), fArr, null);
            this.f22mX[i2] = fArr[0];
            this.f23mY[i2] = fArr[1];
        }
        if (Math.abs(this.f22mX[0]) <= 1.0E-5d && Math.abs(this.f23mY[0]) <= 1.0E-5d) {
            int i3 = min - 1;
            if (Math.abs(this.f22mX[i3] - 1.0f) <= 1.0E-5d && Math.abs(this.f23mY[i3] - 1.0f) <= 1.0E-5d) {
                int i4 = 0;
                float f = 0.0f;
                while (i < min) {
                    float[] fArr2 = this.f22mX;
                    int i5 = i4 + 1;
                    float f2 = fArr2[i4];
                    if (f2 < f) {
        ...
        sb.append(this.f22mX[0]);
        sb.append(",");
        sb.append(this.f23mY[0]);
        sb.append(" end:");
        int i6 = min - 1;
        sb.append(this.f22mX[i6]);
        sb.append(",");
        sb.append(this.f23mY[i6]);
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f22mX.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f22mX[i2]) {
                length = i2;
        ...
        float[] fArr = this.f22mX;
        float f2 = fArr[length] - fArr[i];
        if (f2 == 0.0f) {
            return this.f23mY[i];
        }
        float[] fArr2 = this.f23mY;
        float f3 = fArr2[i];
