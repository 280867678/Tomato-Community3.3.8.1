...
import android.support.p006v8.renderscript.Allocation;
...
public final class ScriptGroup extends BaseObj {
    ...
    /* loaded from: classes2.dex */
    static class C0568IO {
        Allocation mAllocation;
        Script.KernelID mKID;

        C0568IO(Script.KernelID kernelID) {
            this.mKID = kernelID;
        }
    }
    ...
    public void setInput(Script.KernelID kernelID, Allocation allocation) {
        int i = 0;
        while (true) {
            C0568IO[] c0568ioArr = this.mInputs;
            if (i < c0568ioArr.length) {
                if (c0568ioArr[i].mKID == kernelID) {
                    c0568ioArr[i].mAllocation = allocation;
                    if (this.mUseIncSupp) {
                        return;
                    }
                    RenderScript renderScript = this.mRS;
    ...
    public void setOutput(Script.KernelID kernelID, Allocation allocation) {
        int i = 0;
        while (true) {
            C0568IO[] c0568ioArr = this.mOutputs;
            if (i < c0568ioArr.length) {
                if (c0568ioArr[i].mKID == kernelID) {
                    c0568ioArr[i].mAllocation = allocation;
                    if (this.mUseIncSupp) {
    ...
    public void execute() {
        ...
        while (it2.hasNext()) {
            Node next = it2.next();
            Iterator<Script.KernelID> it3 = next.mKernels.iterator();
            while (it3.hasNext()) {
                Script.KernelID next2 = it3.next();
                Iterator<ConnectLine> it4 = next.mInputs.iterator();
                ...
                for (C0568IO c0568io : this.mInputs) {
                    if (c0568io.mKID == next2) {
                        allocation2 = c0568io.mAllocation;
                    }
                }
                Iterator<ConnectLine> it5 = next.mOutputs.iterator();
                ...
                for (C0568IO c0568io2 : this.mOutputs) {
                    if (c0568io2.mKID == next2) {
                        allocation4 = c0568io2.mAllocation;
                    }
                }
    ...
    public static final class Builder {
        ...
        private boolean calcOrderRecurse(Node node, int i) {
            ...
            Iterator<ConnectLine> it2 = node.mOutputs.iterator();
            boolean z = true;
            while (it2.hasNext()) {
        ...
        public ScriptGroup create() {
            ...
            while (i2 < this.mNodes.size()) {
                ...
                while (i5 < node.mKernels.size()) {
                    ...
                    if (!z) {
                        arrayList.add(new C0568IO(kernelID));
                    }
                    if (!z2) {
                        arrayList2.add(new C0568IO(kernelID));
                    }
                    i5++;
                    i4 = i6;
                }
            ...
            scriptGroup.mOutputs = new C0568IO[arrayList2.size()];
            for (int i10 = 0; i10 < arrayList2.size(); i10++) {
                scriptGroup.mOutputs[i10] = (C0568IO) arrayList2.get(i10);
            }
            scriptGroup.mInputs = new C0568IO[arrayList.size()];
            for (int i11 = 0; i11 < arrayList.size(); i11++) {
                scriptGroup.mInputs[i11] = (C0568IO) arrayList.get(i11);
            }
