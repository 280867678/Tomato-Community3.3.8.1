...
public final class ColorStateListInflaterCompat {
    ...
    private static ColorStateList inflate(@NonNull Resources resources, @NonNull XmlPullParser xmlPullParser, @NonNull AttributeSet attributeSet, @Nullable Resources.Theme theme) throws XmlPullParserException, IOException {
        ...
        while (true) {
            ...
            if (next == 2 && depth <= depth2 && xmlPullParser.getName().equals("item")) {
                TypedArray obtainAttributes = obtainAttributes(resources, theme, attributeSet, C0031R.styleable.ColorStateListItem);
                int color = obtainAttributes.getColor(C0031R.styleable.ColorStateListItem_android_color, -65281);
                float f = 1.0f;
                if (obtainAttributes.hasValue(C0031R.styleable.ColorStateListItem_android_alpha)) {
                    f = obtainAttributes.getFloat(C0031R.styleable.ColorStateListItem_android_alpha, 1.0f);
                } else if (obtainAttributes.hasValue(C0031R.styleable.ColorStateListItem_alpha)) {
                    f = obtainAttributes.getFloat(C0031R.styleable.ColorStateListItem_alpha, 1.0f);
                }
                ...
                for (int i4 = 0; i4 < attributeCount; i4++) {
                    ...
                    if (attributeNameResource != 16843173 && attributeNameResource != 16843551 && attributeNameResource != C0031R.attr.alpha) {
                        int i5 = i3 + 1;
    ...
    @ColorInt
    private static int modulateColorAlpha(@ColorInt int i, @FloatRange(from = 0.0d, m5592to = 1.0d) float f) {
